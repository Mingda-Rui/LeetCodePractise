...
/**
 * 3.3 Stack of Plates: Imagine a (literal) stack of plates. If the stack gets too high, it might
 * topple. Therefore, in real life, we would likely start a new stack when the previous stack
 * exceeds some threshold. Implement a data structure SetOfStacks that mimic this. SetOfStacks
 * should be composed of several stacks and should create a new stack once the previous one exceeds
 * capacity. SetOfStacks.push() and SetOfStacks.pop() should behave indentically to a single stack
...
class SetOfStacksShiftPopAt extends SetOfStacks {
  ...
  public int popAt(int index) {
    if (index >= getTotalSize() - 1) return pop();
  ...
  private void shiftByOne(int startStack, int startIndex) {
    ...
    if (currentStackIndex >= startStack + 1) shiftByOne(startStack + 1, 0);
  }
...
class SetOfStacksNonShiftPopAt extends SetOfStacks {
  ...
  public int push(int val) {
    ...
    if (currentStackIndex == subStackSizes.size()) subStackSizes.add(0);
