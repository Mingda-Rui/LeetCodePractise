...
public class LC0222CountCompleteTreeNodes {
  public int countNodes(TreeNode root) {
    ...
    for (int i = 1; i < pow.length; i++) pow[i] = pow[i - 1] * 2;
    int currentCount = 1;
    for (int i = 0; i < depth - 1; i++) currentCount += pow[i];
  ...
  private int countNodes(TreeNode root, int depth, int currentD, int currentCount, int[] pow) {
    if (root == null) return 0;
    if (depth == currentD + 1) return currentCount;
  ...
  public int countNodesShortest(TreeNode root) {
    ...
    else return countNodesShortest(root.left) + countNodesShortest(root.right) + 1;
  }
