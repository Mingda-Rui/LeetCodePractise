...
public class LC0033SearchInRotatedSortedArray {
    public int search(int[] nums, int target) {
        ...
        if (target < nums[smallest] || target > nums[greatest]) return -1;

        if (target >= nums[0]) return binarySearch(nums, 0, greatest + 1, target);
        else return binarySearch(nums, smallest, nums.length, target);
    }
    ...
    private int binarySearch(int[] nums, int start, int end, int target) {
        ...
        if (current > target) return binarySearch(nums, start, mid, target);
        else if (current < target) return binarySearch(nums, mid, end, target);
        else return mid;
    }
