...
public class LC0235LowestCommonAncestorOfABinarySearchTree {
  ...
  public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
    ...
    if (root.val > p.val && root.val > q.val) return lowestCommonAncestor(
      root.left,
      p,
      q
    );
  ...
  public TreeNode lowestCommonAncestorNonBst(
    TreeNode root,
    TreeNode p,
    TreeNode q
  ) {
    ...
    if (p == root || p == left || p == right) return p;
    if (q == root || q == left || q == right) return q;
