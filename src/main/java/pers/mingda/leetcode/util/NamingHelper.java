...
/**
 * In GitHub UI, classes in the same package are sorted by class name initials. If we want the
 * LeetCode problem sorted in the same order with their problem number, we need to covert the
 * problem number to string. Then, make sure GitHub orders those strings in the same way with the
 * problem number's order.<br>
 * `NamingHelper` helps to convert the problem number to a prefix so that we can prefix each class
 * name with the prefix, and those classes will be ordered in UI in the same way with their problem
 * number. It can also convert the prefix back to the problem number.
 */
